...
- **Custom Skills (Slash Commands):** Vault này có các kỹ năng tùy chỉnh nằm trong thư mục `05 Skills/`. Khi user nhắc đến tên các skill này (có hoặc không có dấu `/`), Claude **PHẢI** dùng tool `Read` để đọc file md tương ứng trong `05 Skills/` và thực hiện theo đúng hướng dẫn bên trong.
    - `learn-continue` (hoặc `/learn-continue`) -> `05 Skills/learn-continue.md`
    - `day-update` (hoặc `/day-update`) -> `05 Skills/day-update.md`
    - `brain-setup` (hoặc `/brain-setup`) -> `05 Skills/brain-setup.md`
    - `new-project` (hoặc `/new-project`) -> `05 Skills/new-project.md`
    - `weekly-update` (hoặc `/weekly-update`) -> `05 Skills/weekly-update.md`
