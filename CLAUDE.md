...
3. **The Feynman gate.** A concept reaches `Understood` only when the user explains the *mechanism* in their own words (Bloom L2 — how/why, not just what). A correct result with no mechanism stays `Partial`. Never promote on a shallow answer; never fabricate understanding the user did not demonstrate. **Inline close:** the moment a concept passes the Feynman gate, Claude immediately (a) writes/updates the theory note with `status: Understood` and the new SR schedule, and (b) logs a `[FEYNMAN] <note-name> — <grade> — <today>` line in today's journal. The user never needs to trigger this — it happens in the same turn as the pass.
...
**Auto-close:** After a concept completes its full cycle (theory `Understood` + practice done + output saved if applicable), Claude asks the user: "Bạn muốn học tiếp concept tiếp theo, hay kết thúc buổi học?" If the user chooses to continue, move to the next concept in `GOALS.md`. If the user chooses to end, or if there are no more concepts in the roadmap, Claude immediately runs the session close flow: append a brief summary to today's journal (listing what was covered), update `04 Reviews/Reasoning-Gaps.md` if new gaps surfaced, update the **Current Status** line in the subject's `CLAUDE.md`, and end with a forward question for the next session. The user never needs to manually invoke `/done` — it happens automatically when the session naturally concludes.
...
- **`/learn [subject]`** — start/resume a session: load the subject's context, review the notes that are due (Socratic + grade + reschedule), then teach the next concept challenge-first. On a brand-new subject with no notes yet, run a short placement across the roadmap first, then teach. After each concept passes the Feynman gate, Claude automatically updates its note and schedule inline (Rule 3). On session wind-down (practice done + no more concepts), Claude auto-closes: journal, Reasoning-Gaps, Current Status. The user only calls `/done` to cut a session short mid-concept.
