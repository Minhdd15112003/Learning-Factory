...
Toàn bộ `gcp-document/` là **một** vault Obsidian và **một** project Claude (git root). Mỗi chủ đề học là một **brain** — một thư mục cấp 1 bên trong, có hiến pháp riêng nhưng dùng chung hạ tầng.

```
gcp-document/                ← vault + git root
...
> **Mở vault trong Obsidian ở thư mục `gcp-document/`** (không phải `GCP/`). Tất cả brain nằm trong một graph, dùng chung plugin (gồm Spaced Repetition).
...
- **Claude "kỹ thuật" quá** thì nói *"Socratic"* — nó quay lại đặt câu hỏi dẫn dắt.
