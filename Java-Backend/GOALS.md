...
- Trực tiếp học, áp dụng và thực hành **TOÀN BỘ CÁC CÔNG NGHỆ BACKEND** vào **1 DỰ ÁN DUY NHẤT** tên là `commerce-fulfillment-system` từ con số 0 lên Microservices hoàn chỉnh.
...
## 🚀 Định Hướng 1 Dự Án Duy Nhất: E-commerce Order & Fulfillment Backend (`commerce-fulfillment-system`)

Từ **Giai đoạn 2 trở đi**, toàn bộ kiến thức lý thuyết, kiến trúc, công cụ và Design Patterns sẽ được **áp dụng trực tiếp vào 1 dự án duy nhất**: `commerce-fulfillment-system`.
...
_Mục tiêu:_ Khởi tạo dự án duy nhất `commerce-fulfillment-system` bằng DDD + Hexagonal Architecture với đầy đủ các công cụ và chuẩn API.
...
- [ ] **Strategic Domain-Driven Design (DDD) & System Design:**
  ...
  - Phân chia các **Bounded Contexts**: _Catalog Context_, _Order Context_, _Inventory Context_, _Payment Context_, _Fulfillment Context_, _Notification Context_.
  - Xây dựng **Context Map** (sơ đồ giao tiếp giữa các Bounded Contexts).
- [ ] **Tactical DDD & Hexagonal Core (Ports & Adapters):**
  - Pure Java Domain Core (0 phụ thuộc Spring/DB): Aggregates (`Order`, `Inventory`), Entities, Value Objects (`Money`, `SKU`), Domain Events (`OrderPlacedEvent`, `InventoryReservedEvent`), Domain Services.
  - Ports: Inbound Use Cases (`PlaceOrderUseCase`), Outbound Ports (`OrderRepositoryPort`, `InventoryPort`, `PaymentGatewayPort`).
...
- [ ] **Design Patterns cài đặt vào dự án:** [[Dependency Injection]], [[Chain of Responsibility]] (Security Filter Chain), [[Strategy]] (áp dụng chính sách giảm giá / phí vận chuyển), [[Adapter]] (tích hợp mock cổng thanh toán).

---

### Giai đoạn 3 — Database Optimization, Caching, Event-Driven & Storage Hardening

_Mục tiêu:_ Tối ưu hóa CSDL quan hệ, NoSQL, Caching, Media Storage và Messaging bất đồng bộ cho Monolith.
...
- [ ] **Database & ORM (PostgreSQL & MySQL & Spring Data JPA):**
  ...
  - Concurrency & Transaction Control: Transaction Isolation Levels, Optimistic Locking (`@Version`), Pessimistic Locking chống Race Condition khi đặt hàng/giữ tồn kho trùng thời điểm.
...
- [ ] **Object Storage & Media Service (MinIO & AWS S3 & CloudFront):**
  ...
  - Tích hợp **AWS S3** và **AWS CloudFront** với Presigned URL để truy xuất media hiệu quả (ảnh sản phẩm).
- [ ] **Event-Driven Architecture & Messaging (Kafka & RabbitMQ):**
  ...
  - **Apache Kafka:** Event Streaming chịu tải lớn cho luồng sự kiện đặt hàng & thông báo.
...
_Mục tiêu:_ Tách các Hexagonal Bounded Contexts thành các Microservices độc lập.
...
- [ ] **Bóc Tách Microservices (Strangler Fig Pattern):**
  - Tách Monolith thành các Microservices độc lập:
    ...
    5. `fulfillment-service` (Port 8085)
...
- [ ] **High-Performance Communication (gRPC & Protobuf):**
  - **gRPC & Protocol Buffers:** Xây dựng luồng giao tiếp nhị phân tốc độ cao, độ trễ cực thấp giữa `order-service` và `inventory-service`.
- [ ] **Distributed Transactions & Architectural Patterns:**
  - **Saga Pattern (Orchestration / Choreography):** Quản lý giao dịch phân tán nhiều bước khi đặt hàng xuyên nhiều service (Order → Inventory → Payment → Fulfillment).
...
_Mục tiêu:_ Đóng gói, triển khai cluster Microservices và thiết lập hệ thống giám sát phân tán chuẩn Production.
